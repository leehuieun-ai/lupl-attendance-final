...
export function requestToDays(req:LeaveRequest){if(req.request_type==="hourly")return req.amount_hours!=null?Number(req.amount_hours)/8:0;if(req.amount_days!=null)return Number(req.amount_days);if(req.amount_hours!=null)return Number(req.amount_hours)/8;if(req.request_type==="half_am"||req.request_type==="half_pm")return .5;return countWeekdays(req.start_date,req.end_date)}
